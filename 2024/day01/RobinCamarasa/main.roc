...
parse = \content ->
    ...
    lines
        ...
        |> List.keepOks (\list -> when (List.get list 0, List.get list 1) is
                                    (Ok left, Ok right) -> when (Str.toI64 left, Str.toI64 right) is
                                                                    (Ok leftNum, Ok rightNum) -> Ok {left: leftNum, right: rightNum}
                                                                    _ -> Err 1
                                    _ -> Err 1
                    )
        |> List.walk { left: [], right: [] } (\{left: leftList, right: rightList}, {left: left, right: right} -> {left: List.append leftList left, right: List.append rightList right})
...
expect solve {content: testContent, part: 1} == "Part 1: 11" # TODO 3 Adapt test
